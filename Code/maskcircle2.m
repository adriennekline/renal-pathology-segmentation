...
function m = maskcircle2(I,type, x_position, y_position, glom_radius_proportion)
...
%[rows_addie, columns_addie, numberOfColorChannels_addie] = size(temp);
...
switch lower (type)
    ...
    case 'medium'
        r = glom_radius_proportion*200;
%         r = min(min(cx,p-cx),min(cy,q-cy));
%         r = max(2/3*r,25);
        n = zeros(size(x));
        %n((x-cx).^2+(y-cy).^2<r.^2) = 1;
%         n((x-100).^2+(y-100).^2<r.^2) = 1;
        n((x-x_position).^2+(y-y_position).^2<r.^2) = 1;
        m(1:size(n,1),1:size(n,2)) = n;
        %m((x-cx).^2+(y-cy).^2<r.^2) = 1;
    case 'large'
        r = min(min(cx,p-cx),min(cy,q-cy));
